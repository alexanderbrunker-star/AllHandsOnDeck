...
struct HostSessionView: View {
    ...
    var body: some View {
        ...
        .alert("Session expired", isPresented: .init(
            ...
            Button(DesignLabels.close) { closeHostSession() }
        } message: {
            Text(DesignLabels.sessionExpiredMessage)
        }
    ...
    private var bottomBar: some View {
        VStack(spacing: 8) {
            ...
            if !vm.pendingCaptureRequests.isEmpty {
                Text(DesignLabels.requestFromCrew)
            ...
            HStack(spacing: 10) {
                if vm.countdown.state.isActive {
                    PrimaryButton(title: DesignLabels.cancel, systemImage: "xmark", style: .destructive) {
                        Task { await vm.cancelCountdown() }
                    }
                    .accessibilityIdentifier("host_cancel_countdown")
                } else {
                    PrimaryButton(
    ...
    private var settingsSheet: some View {
        VStack(spacing: 16) {
            ...
            VStack(alignment: .leading, spacing: 10) {
                Text(DesignLabels.whoCanTrigger)
                    .font(.system(size: 13, weight: .heavy, design: .rounded))
                    .tracking(1.0)
                    .foregroundStyle(Theme.mist)
